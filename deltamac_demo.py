...
raw_A = st.text_area("Paste everything about Understanding A here", height=120, placeholder="Lab result A: 5.5... or 'I want to order pizza tonight...'")

st.subheader("Understanding B - Raw Information")
raw_B = st.text_area("Paste everything about Understanding B here", height=120, placeholder="Lab result B: 32... or 'I want to cook at home...'")
...
st.subheader("Answer these 8 questions for both sides")
...
for i, q in enumerate(questions):
    ...
    with col1:
        score_a = st.slider(f"A: {q}", 0.0, 1.0, 0.7, 0.05, key=f"qa_{i}")
        scores_A.append(score_a)
    with col2:
        score_b = st.slider(f"B: {q}", 0.0, 1.0, 0.7, 0.05, key=f"qb_{i}")
...
if st.button("🔥 CALCULATE THE RIGHT OF RIGHT", type="primary", use_container_width=True):
    ...
    bias_A = np.array([0.05] * 8)  # Small default bias
    bias_B = np.array([0.05] * 8)
    
    A_clean = np.maximum(0.0, A - bias_A * 0.15)
    B_clean = np.maximum(0.0, B - bias_B * 0.15)
